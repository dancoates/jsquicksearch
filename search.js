...
(function(window, document){
    ...
    // Public methods will extend the prototype chain
    // public variables should be set within the 
    // search constructor and will be properties of
    // the particular instance of Search
    var _public = {};

    // util object holds utility functions. These are
    // also private functions that are not exposed to 
    // the implementation
    var _util = {};
    ...
    var Search = function(data, options, debug){
        ...
        // All variables and methods under the _public object
        // from this point are related to the instance
        // not the Search prototype
        _public = this;
    ...
    _public.search = function(searched){
        ...
        for (i = 0; i < words.length; i++) {
            ...
            var len = word.length;
            if(len < _public.settings.minLength) continue;
            var dict = _public.dict.search;
            if(dict) {
                matches = matches.concat(_private.getMatches(dict, word, len, 0, []));
            }
        }
    ...
    _private.assignWeights = function(results) {
        for (var i = 0; i < results.length; i++) {
            ...
            for (var j = 0; j < result.matches.length; j++) {
                ...
                var baseWeight = _util.findByKey(_public.settings.searchAreas, "key", match.key).weighting;
                var proportion = match.searchedWord.length / match.word.length;
    ...
    _private.highlightResults = function(results) {
        for (var i = 0; i < results.length; i++) {
            ...
            for (var j = 0; j < matches.length; j++) {
                ...
                rangesPerArea[match.key].push(_private.normalizeHighlightRange(startPos, endPos, replacements));
            }
    ...
    _private.mergeRanges = function(ranges) {
        var merged = [];
        for (var i = 0; i < ranges.length; i++) {
            ...
            for (var j = 0; j < merged.length; j++) {
                var mergedLower = merged[j][0];
                var mergedUpper = merged[j][1];
                // Upper merge
                if(
                    lower >= mergedLower &&
                    lower <= mergedUpper &&
    ...
    _util.findByKey = function(arr, key, search) {
        var result;
